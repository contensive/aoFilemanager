...
# Display build info
$version = Get-ContensiveVersion -DeploymentRoot 'C:\Deployments\aoFilemanager'
Write-Host ""
Write-Host "========================================"
Write-Host "  File Manager Build"
...
Invoke-ContensiveBuild `
    -CollectionName    'File Manager' `
    -CollectionPath    "$projectRoot\collections\File Manager" `
    -SolutionPath      "$projectRoot\aoFileManager\aoFileManager.sln" `
    -BinPath           "$projectRoot\aoFileManager\aoFileManager\bin\$Configuration\netstandard2.0" `
    -Configuration     $Configuration `
    -DeploymentRoot    'C:\Deployments\aoFilemanager' `
    -CleanFolders      @(
                           "$projectRoot\aoFileManager\aoFileManager\bin"
                           "$projectRoot\aoFileManager\aoFileManager\obj"
                       ) `
